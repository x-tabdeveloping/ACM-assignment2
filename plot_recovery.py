...
fig = make_subplots(
    ...
    vertical_spacing=0.1,
    horizontal_spacing=0.01,
    subplot_titles=[f"$N={n}$" for n in ns] + ["No Reaction Times"],
)
records = []
colors = px.colors.qualitative.Bold
for file in files:
...
fig = fig.update_layout(
    template="plotly_white", margin=dict(l=5, r=5, b=5, t=20), width=1100, height=300
)
fig.show()

fig.write_image(fig_dir.joinpath("n_trials_recovery.png"), scale=2)
